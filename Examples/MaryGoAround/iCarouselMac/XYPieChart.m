...
//        self.backgroundColor = [UIColor clearColor];
        _pieView = [[NSView alloc] initWithFrame:frame];
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if(self)
    {
        _pieView = [[NSView alloc] initWithFrame:self.bounds];
//        [_pieView setBackgroundColor:[UIColor clearColor]];
        [self addSubview:_pieView];//atIndex:0];
...
{
    [_pieView setCenterX:pieCenter.x];
    [_pieView setCenterY:pieCenter.y];
...
//        [_pieView setUserInteractionEnabled:NO];
        ...
        __unsafe_unretained NSMutableArray *layersToRemove = nil;
...
//            [_pieView setUserInteractionEnabled:YES];
        ...
        for(int index = 0; index < sliceCount; index ++)
        {
            ...
            if(!color)
            {
                color = [NSColor colorWithDeviceHue:((index/8)%20)/20.0+0.02 saturation:(index%8+3)/10.0 brightness:91/100.0 alpha:1];
            }
...
-(void) mouseDown:(NSSet *)touches WithEvent:(NSEvent*) event {
...
- (SliceLayer *)createSliceLayer
{
    ...
    CGSize size = CGSizeMake(24.0, 24.0); //, <#CGFloat height#>)[@"0" sizeWithFont:self.labelFont];
    [CATransaction setDisableActions:YES];
